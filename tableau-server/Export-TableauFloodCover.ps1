...
.EXAMPLE
    .\Export-TableauFloodCover.ps1 -TableauPassword $env:TABLEAU_PASSWORD
.EXAMPLE
    .\Export-TableauFloodCover.ps1 -TableauPassword $env:TABLEAU_PASSWORD -ExportCount 3
.EXAMPLE
    .\Export-TableauFloodCover.ps1 -TableauPassword $env:TABLEAU_PASSWORD -DryRun
#>
    ...
    if ($DryRun) {
        ...
        try {
            $remoteResult = Invoke-Command -Session $session -ArgumentList @(
                ...
                # Resolve tabcmd.exe — standalone install on 2022.3+ lives under \<version>\extras\Command Line Utility\
                $tabcmdExe = (Get-Command tabcmd -ErrorAction SilentlyContinue).Source
                if (-not $tabcmdExe) {
                    $searchRoots = @(
                        'C:\Program Files\Tableau\Tableau Server',
                        'D:\Tableau'
                    ) | Where-Object { Test-Path $_ }
                    $tabcmdExe = $searchRoots |
                        ForEach-Object { Get-ChildItem -Path $_ -Filter 'tabcmd.exe' -Recurse -ErrorAction SilentlyContinue } |
                        Sort-Object FullName -Descending |
                        Select-Object -First 1 -ExpandProperty FullName
                }
                if (-not $tabcmdExe) {
                    throw "tabcmd.exe not found on $env:COMPUTERNAME (checked PATH, C:\Program Files\Tableau\Tableau Server\, D:\Tableau\)"
                }
                Write-Host "Using tabcmd: $tabcmdExe"
                ...
                try {
                    ...
                    $loginResult = & $tabcmdExe @loginArgs 2>&1
