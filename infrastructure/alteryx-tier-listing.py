...
def parse_workflow(xml_path):
    """Parse a single .yxmd file and return a structured dict."""
    result = {
        "file": str(xml_path),
        "name": "",
        "version": "",
    ...
    # Classify tier
    result["tier"] = classify_tier(list(result["tools"].keys()), len(result["macros"]) > 0)
...
def build_report(results, output_path):
    ...
    summary_data = [
        ...
        ["", ""],
        ["Total Workflows Analysed", total],
        ["", ""],
        ["Tier", "Count"],
    ]
