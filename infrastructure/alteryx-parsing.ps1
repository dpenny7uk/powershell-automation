...
foreach ($wf in $allWorkflows) {
    ...
    $lastJob = $null
    try {
        $jobs = Invoke-RestMethod -Uri "$BaseUrl/v3/workflows/$($wf.id)/jobs?skip=0&take=1" -Headers $headers -Method Get
    ...
    $results.Add([PSCustomObject]@{
        ...
        lastJobCreateDate   = if ($lastJob) { $lastJob.createDateTime } else { "" }
        lastJobCompleteDate = if ($lastJob) { $lastJob.completionDateTime } else { "" }
    })
